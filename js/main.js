...
function initState(presetData) {
  AppState.data = presetData || loadData();
...
function initGradeFormHandler() {
  document.getElementById('gradeForm').addEventListener('submit', (e) => {
    ...
    const note = Math.round(parseFloat(document.getElementById('gradeValue').value) * 100) / 100;
...
async function handleDeleteGrade(grade) {
  const confirmed = await confirmDialog(
    `Note "${grade.type}" (${Number(grade.note).toFixed(2)}) für ${grade.subject} löschen?`,
    { title: 'Note löschen' }
  );
  if (!confirmed) return;
...
async function boot() {
  if (!CLOUD_SYNC_ENABLED) document.getElementById('appRoot').hidden = false;
  ...
  if (CLOUD_SYNC_ENABLED) {
    document.getElementById('footerNote').textContent =
      'Deine Daten werden synchronisiert und sind auf allen angemeldeten Geräten verfügbar.';

    const user = await initAuth(); // zeigt bei Bedarf das Login-Gate und wartet auf Anmeldung
    setSyncUser(user.id);

    const cloudData = await pullCloudData(user.id);
    if (cloudData) {
      presetData = cloudData;
      cacheDataLocally(presetData); // als Offline-Cache, ohne einen unnötigen Cloud-Push auszulösen
    } else {
      presetData = loadData();
      await pushCloudData(user.id, presetData); // erster Login: lokalen Stand in die Cloud übernehmen
    }
  }

  initState(presetData);
  renderAll();
}
