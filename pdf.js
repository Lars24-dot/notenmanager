...
function exportSemesterPdf() {
  ...
  doc.setFontSize(11);
  doc.text(`Schuljahr: ${AppState.year} — Semester: ${AppState.semester}`, marginX, 28);
  ...
  const rows = branch.subjects.map(subject => {
    ...
    const gradesText = grades.length
      ? grades.map(g => `${g.type} ${Number(g.note).toFixed(1)} (Gew. ${g.weight})`).join(', ')
      : '—';
  ...
  const signatureY = Math.min(finalY + 40, doc.internal.pageSize.getHeight() - 20);
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(10);
  doc.line(marginX, signatureY, marginX + 70, signatureY);
  doc.text('Ort, Datum, Unterschrift', marginX, signatureY + 5);

  const fileName = `Notenuebersicht_${AppState.year.replace('/', '-')}_${AppState.semester}_${AppState.schoolType}.pdf`;
  doc.save(fileName);
